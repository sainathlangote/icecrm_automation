...
public class FindBrokenlinks extends BaseClass {
	...
	public void verifybrokenlinks() {
		List<WebElement> links = driver.findElements(By.tagName("a")); // firstly findout the tag whre links Associated
		for (int i = 0; i < links.size(); i++) { // iterate one by one
			WebElement element = links.get(i); // get one by one link
			String url = element.getAttribute("href"); // find Link associated in href atribute
